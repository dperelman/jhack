...
public class MainGUI implements ActionListener, WindowListener {
	...
	private void initGraphics() {
		...
		for (int i = 0; i < compTypes.length; i++) {
			...
			if ((formatPref == null && i == 0)
					|| (formatPref != null && formatPref
							.equals(compTypes[i][0])))
	...
	private boolean loadRom(File loc) {
		...
		if (rom.getFilePath().getAbsoluteFile().equals(
	 ...
	 * @param modClass
	 *            Class you want to show, must extend HackModule.
	 * @param input
	 *            Input to send to class's {@link HackModule#show(Object)}.
	 ...
	 * @param modClass
	 *            Name of the class you want to show, must extend HackModule.
	 * @param input
	 *            Input to send to class's {@link HackModule#show(Object)}.
	 * @return True if successful, false if module not loaded or input invalid.
	 * @throws ClassNotFoundException
	 *             On named class not existing.
	 */
