 ...
 * The appearance is based on the toolset in Mr. Accident's EBSE.
 * This allows the user to control all of the toolset options.
 * 
 * @author AnyoneEB
 */
public class DrawingToolset extends JComponent implements Toolset
{
	...
	private void initGraphics(boolean flip)
	{
		...
		settings.add(
			HackModule.getLabeledComponent("Fill: ", fillSelector),
			BorderLayout.NORTH);
		...
		settings.add(
			HackModule.getLabeledComponent(
				"Curve Size: ",
				roundedRectRad,
				"Curve width & height for rounded rectangles"),
			BorderLayout.SOUTH);
