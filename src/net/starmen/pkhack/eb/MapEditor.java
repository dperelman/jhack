    ...
    PropertyChangeListener, MouseListener, AdjustmentListener
{
    ...
    public static final String[][][] menuNames = {
        ...
        {{"Mode", "m"}, {"Map View (Text)", "1"}, {"Map View", "2"}, {"Map Edit", "3"}},
        {{"Help", "h"}, {"About", "a"}}};
	...
	gfxcontrol.setPreferredSize(new Dimension(tilewidth * screen_width + screen_width + 1, tileheight * screen_height + screen_height + 1));
    ...
    protected void init()
    {
        ...
        {
            System.out.println("Test row tile " + i + ": "
    ...
    {
        // System.out.println("source: " + ae.getSource() + " value: " + ae.getValue());
	...
	private int startedity;
        ...
        // = change sector vars too), draw map (0=no, 1=text tiles, 2=gfx tiles),
        //   allow map editing}
            {2, 1, 0}, {2, 2, 0}, {2, 2, 1}};
...
//                            + mapcontrol.getLocalTileset(i2 + x, i + y));
			...
			tile_tile = row2draw[i2] | (mapcontrol.getLocalTileset(i2 + x, i + y) << 8);
			// tile_pal = tsetPal[1]; // TileEditor.tilesets[mapcontrol.getDrawTileset(tsetPal[0])].getPaletteNum(tsetPal[0], tsetPal[1]);
			tile_pal = TileEditor.tilesets[mapcontrol.getDrawTileset(tsetPal[0])].getPaletteNum(tsetPal[0], tsetPal[1]);
			// System.out.println("Tileset: " + tile_set + " Tile: " + tile_tile + " Palette: " + tile_pal);
			this.loadTileImage(tile_set, tile_tile, tile_pal);
                        /*g.drawImage(TileEditor.tilesets[mapcontrol
                            .getDrawTileset(tsetPal[0])]
                            .getArrangementImage(
                                row2draw[i2]
                                    | (mapcontrol
                                        .getLocalTileset(i2 + x, i + y) << 8),
                                TileEditor.tilesets[mapcontrol
                                    .getDrawTileset(tsetPal[0])].getPaletteNum(
                                    tsetPal[0], tsetPal[1]), false),
                            this.startx + (i2 * this.tilewidth), this.starty
                                + (i * this.tileheight), this.tilewidth,
                            this.tileheight, this);*/
			g.drawImage(this.tile_images[tile_set][tile_tile][tile_pal], this.startx + (i2 * this.tilewidth), this.starty + (i * this.tileheight), this.tilewidth, this.tileheight, this);
            ...
            int woh, int startxory, int xory, int tilewoh)
        {
            ...
            else
            {
                ...
                {
                    //System.out
                    //    .println("The sector being drawn DOES extend horizontally past the border.");
                    //System.out.println("Expected end of sector: "
                ...
                {
                    //System.out
                    //    .println("The sector being drawn DOES NOT extend horizontally past the border.");
                    draw_sector_woh = sector_woh * tilewoh;
                }
            }
        ...
        {
            g2d.setPaint(Color.black);
            Rectangle2D.Double border = new Rectangle2D.Double(this.startx - 1,
                this.starty - 1, (this.width * this.tilewidth) + 1,
        ...
        public void loadTileImage(int loadtset, int loadtile, int loadpalette) {
            // if (!((this.tile_images == null) && (this.tile_images[loadtset][loadtile][loadpalette] == null)))
            if (this.tile_images[loadtset][loadtile][loadpalette] == null)
            {
                // System.out.println("Now loading tile " + loadtset + "/" + loadtile + "/" + loadpalette + " and there are " + TILESET_NAMES.length + " unique tilesets");
	        // load tile images into a local array tile_images
                this.tile_images[loadtset][loadtile][loadpalette] = TileEditor.tilesets[loadtset].getArrangementImage(loadtile, loadpalette);
            }
        }
    ...
    public static class EditingGUI
    {
	private int gui_x;
	...
	private void drawEditGUI(Graphics g, Graphics2D g2d)
	{
		...
		int tile_pal = TileEditor.tilesets[mapcontrol.getDrawTileset(tsetPal[0])].getPaletteNum(tsetPal[0], tsetPal[1]);
		int tile_tile;
		...
		for (int i = 0; i < this.width; i++)
		    ...
		    {
		    tile_tile = (editUI_x + i) * (i2 + 1) | (mapcontrol.getLocalTileset(i2 + x, i + y) << 8);
		    gfxcontrol.loadTileImage(tile_set, tile_tile, tile_pal);
                    g.drawImage(this.tile_images[tile_set][tile_tile][tile_pal], this.starteditx + (i * this.tilewidth), this.startedity + ((i2 + 1) * this.tileheight), this.tilewidth, this.tileheight, this);
                    g2d.draw(new Rectangle2D.Double(this.starteditx+ (i * this.tilewidth), this.startedity + ((i2 + 1) * this.tileheight), tilewidth, tileheight));
                    }
		}
            } else {
	       g2d.setPaint(Color.black);
	           ...
	           for (int i2 = 0; i2 < 2; i2++)
		   {
		       g2d.draw(new Rectangle2D.Double(this.starteditx
	                ...
	                + ((i2 + 1) * this.tileheight), tilewidth, tileheight));
                   }
	       }
	    }
	}
    ...
    public static class EbMap
    {
 	...
 	// this.all_addresses = new int[] { 0x160200, 0x162A00, 0x165200,
        ...
        {
            int address = this.all_addresses[row] + start;
        ...
        {
            int address = this.tsetpal_address
        ...
        public int getDrawTileset(int mapTset)
        {
            ...
            int address = this.tsettbl_address + (mapTset * 2);
        ...
        {
            int address = this.localtset_address + ((glty / 8) * (this.width + 1)) + gltx;
            if(((glty / 4) % 2) ==1) address += 0x3000;
